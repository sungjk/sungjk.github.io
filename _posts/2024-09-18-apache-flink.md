...
쏟아지는 데이터를 실시간으로 활용할 수 있는 환경이 갖추어지니 할 수 있는게 정말 많아졌다. 단순히 Apache Flink라는 새로운 도구에 익숙해진 건 단편적인 예시일 뿐, 데이터에 대해 눈이 떠졌다랄까. 데이터 엔지니어 동료와 처음 티타임을 할 때 데이터에 무지한 나의 모습을 나 스스로 제 3자의 시각에서 바라보고 있는 느낌이 들었고 머릿속에서는 새로운 터널에 진입한 느낌이 들었다. 짧게는 3개월, 길게는 1년 뒤에 달라져 있을 내 모습이 상상되기도 했다. 멋진 동료 덕분에 성장하고 있다는걸 느낄 수 있었고 도움이 필요하면 나도 많이 도와주고 싶다는 생각이 들었다. 

이제 백엔드 엔지니어로서 다룰 수 있는 연장이 하나 더 늘어났다. 경계해야 할 건 모든게 다 못으로 보이지 않게 만드는 것. 실시간으로 데이터를 처리할 수 있는 능력이 생겼으니 모든 곳에 다 스트림 앱을 갖다 붙이고 싶을 수 있다. 그래서 이 도구로 해결하기에 적당한 문제가 무엇인지 알고 있는게 중요하다. 처리할 데이터 소스와 싱크를 지원하는지 알아보고, 실시간성(Real-time) 그리고 Window 기반으로 이벤트를 처리해야 할 문제에 Flink를 추천하고 싶다. 실시간으로 발생하는 거래 데이터가 정상인지, 이상거래는 아닌지 [State](https://flink.apache.org/what-is-flink/flink-applications/#state)와 [Time](https://flink.apache.org/what-is-flink/flink-applications/#time)을 활용해서 판단할 수 있다.

마지막으로, 한국에도 Flink를 사용하는 회사들이 점점 많아지고 있는것 같은데 기술 교류와 레퍼런스가 많아졌으면 좋겠다. 생태계는 계속해서 발전하고 있다는 느낌이 드는데, 대부분 비슷하겠지만 구체적으로 어떤 니즈를 가지고 이 기술을 도입해서 사용하고 있는지가 궁금하다. 자주, 더 많은 기술 공유가 있기를!
